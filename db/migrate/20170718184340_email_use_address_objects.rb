class EmailUseAddressObjects < ActiveRecord::Migration
      ...
      group.each do |obj|
        ...
        begin
          address_fields.keys.each do |key|
            if obj[key].present? && obj.send(address_fields[key].keys.first).blank?
  ...
  def down
    ...
    if ActiveRecord::Base.connection.column_exists?(:cybox_email_messages, :x_ip_cybox_object_id)
      remove_column :cybox_email_messages, :x_ip_cybox_object_id, :string
    end
